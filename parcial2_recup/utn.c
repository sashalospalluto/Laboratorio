...
int isValidFecha(char* charRecibido)
{
    int retorno=1;  // para las funciones isValid arranco con verdadero y cambio cuando encuentro un error
    int i;
    for(i=0;charRecibido[i]!='\0';i++)
    {
        if((charRecibido[i]<'0' || charRecibido[i]>'9') && (charRecibido[i]!='/'))
        {
            retorno=0;
            break;
        }
...
int utn_validaFecha(char* fecha)
{
    int retorno=-1;
    int auxDia;
    int auxMes;
    ...
    if (isValidFecha(fecha)==1)
    {
        if (sscanf(fecha, "%2d/%2d/%4d", &auxDia, &auxMes, &auxAgno) == 3)
        {
            switch(auxMes)
            {
                case 1:
